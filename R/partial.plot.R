...
partial.plot.randomForest <-
  function(x, pred.data, x.var, add=FALSE, n.pt=min(nrow(pred.data), 51),
           rug=TRUE, ...) {
    if(x$type != "regression")
      stop("Sorry, partial.plot is only implemented for regression.")
    if(is.null(x$forest))
    ...
    if(is.factor(xv)) {
      ...
      if(rug) rug(quantile(xv, seq(.1, .9, by=.1)), side=1)
    }
